...

{
  ...
  for (t in 1:nt){
    for (n in 1:nsubj) {
      ...
      like1 = exp(normal_lpdf(y[n, t] | mu[1] , sig_e[1])); // local evidence for class 1
      like2 = exp(normal_lpdf(y[n, t] | mu[2] , sig_e[2])); // local evidence for class 2
...
for (n in 1 : nsubj){
  for (t in 1:nt) {
    ps = pred[n, t] * exp(normal_lpdf(y[n, t] | mu[1], sig_e[1])) +
         (1 - pred[n, t]) * exp(normal_lpdf(y[n, t] | mu[2], sig_e[2]));
    target += log(ps);
    }
  }
}
